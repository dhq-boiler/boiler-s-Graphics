...
public class LogResolvesUnityContainerExtension : UnityContainerExtension
{
    ...
    private class LoggingStrategy : BuilderStrategy
    {
        public override void PreBuildUp(ref BuilderContext context)
        {
            ...
            var registrationNameWithParenthesesOrNothing =
                string.IsNullOrEmpty(registrationName) ? "" : $"({registrationName})";
            LogManager.GetCurrentClassLogger()
                .Debug($"Resolving [{registrationType}{registrationNameWithParenthesesOrNothing}] => [{resolvedType}]");
        }
    }
}