...
public static class OperationExtensions
{
    ...
    /// </summary>
    public static IMergeableOperation GenerateSetPropertyOperation<TProperty>(this object @this, string propertyName,
        TProperty newValue, TimeSpan timeSpan)
    {
        var nullableObj = FastReflection.GetProperty(@this, propertyName);
        var oldValue = nullableObj != null ? (TProperty)nullableObj : default;

        return GenerateAutoMergeOperation(@this, propertyName, newValue, oldValue,
            $"{@this.GetHashCode()}.{propertyName}", timeSpan);
    }

    public static IMergeableOperation GenerateSetStaticPropertyOperation<TProperty>(this Type @class,
        string propertyName, TProperty newValue, TimeSpan timeSpan)
    {
        var nullableObj = FastReflection.GetStaticProperty(@class, propertyName);
        var oldValue = nullableObj != null ? (TProperty)nullableObj : default;

        return GenerateAutoMergeOperation(@class, propertyName, newValue, oldValue,
            $"{@class.GetHashCode()}.{propertyName}", timeSpan);
    }
    ...
    public static IMergeableOperation GenerateSetStaticPropertyOperation<TProperty>(this Type @class,
        string propertyName, TProperty newValue)
    {
        return GenerateSetStaticPropertyOperation(@class, propertyName, newValue, Operation.DefaultMergeSpan);
    }

    public static IMergeableOperation GenerateSetPropertyOperation<T, TProperty>(this T @this,
        Expression<Func<T, TProperty>> selector, TProperty newValue)
    {
    ...
    ///     マージ可能なオペレーションを作成する
    /// </summary>
    public static IMergeableOperation GenerateAutoMergeOperation<TProperty, TMergeKey>(this object @this,
        string propertyName, TProperty newValue, TProperty oldValue, TMergeKey mergeKey, TimeSpan timeSpan)
    {
    ...
    public static IMergeableOperation GenerateAutoMergeOperation<TProperty, TMergeKey>(this Type @class,
        string propertyName, TProperty newValue, TProperty oldValue, TMergeKey mergeKey, TimeSpan timeSpan)
    {
        return new MergeableOperation<TProperty>(
...
public static class ListOperationExtensions
{
    ...
    {
        return new InsertOperation<T>(@this, value, message: $"Add {value} to {@this}");
    }
    ...
    {
        return new RemoveOperation<T>(@this, value, $"Remove {value} from {@this}");
    }
    ...
    {
        return new RemoveAtOperation(@this, index);
    }
    ...
    {
        return ToAddRangeOperation(@this, values as IEnumerable<T>);
    }
    ...
    {
        return ToRemoveRangeOperation(@this, values as IEnumerable<T>);
    }
    ...
    {
        return new ClearOperation<T>(@this);
    }
}
