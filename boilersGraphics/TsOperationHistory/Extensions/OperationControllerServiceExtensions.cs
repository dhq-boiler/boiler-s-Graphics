...
public static class OperationControllerServiceExtensions
{
    ...
    public static void ExecuteSetProperty<T, TProperty>(this IOperationController controller, T owner,
    ...
    public static void ExecuteSetPropertyWithEnforcePropertyType<T, TProperty>(this IOperationController controller,
        T owner, string propertyName, object value)
    {
        var operation = owner
    ...
    public static void ExecuteSetStaticProperty<TProperty>(this IOperationController controller, Type @class,
        string propertyName, TProperty value)
    {
    ...
    public static void ExecuteDispose<T>(this IOperationController controller, T disposing, Action regenerateAction)
        where T : IDisposable
    {
    ...
    public static IDisposable BindPropertyChanged<T>(this IOperationController controller, INotifyPropertyChanged owner,
        string propertyName, bool autoMerge = true)
    {
        if (propertyName.IndexOf(".") != -1)
        {
            ...
            DescendentPropertyNameChain(owner, propertyName, out intermediateValue, out prevValue,
                out bottomLayerPropertyName);
            ...
            void PropertyChanged(object sender, PropertyChangedEventArgs args)
            {
                ...
                if (args.PropertyName == bottomLayerPropertyName)
                {
                    ...
                    var newValue = FastReflection.GetProperty<T>(owner, propertyName);
                    var operation = owner
                        .GenerateAutoMergeOperation(propertyName, newValue, prevValue,
                            $"{sender.GetHashCode()}.{propertyName}", Operation.DefaultMergeSpan);

                    if (autoMerge) operation = operation.Merge(controller);
    ...
    private static void DescendentPropertyNameChain<T>(INotifyPropertyChanged owner, string propertyName,
        out INotifyPropertyChanged intermediateValue, out T prevValue, out string bottomLayerPropertyName)
    {
        bottomLayerPropertyName = propertyName.Substring(propertyName.IndexOf(".") + 1);
        intermediateValue =
            FastReflection.GetProperty<INotifyPropertyChanged>(owner,
        ...
        if (bottomLayerPropertyName.IndexOf(".") != -1)
            DescendentPropertyNameChain(intermediateValue, bottomLayerPropertyName, out intermediateValue,
                out prevValue, out bottomLayerPropertyName);
    }
}