...
{
    private static readonly ConcurrentDictionary<Type, ConcurrentDictionary<string, IAccessor>> Cache = new();

    private static MethodInfo[] _regisMethodInfo;
    private static readonly Dictionary<int, MethodInfo> _methodCache = new();
    ...
    private static IAccessor CreateIAccessor(object _object, string propertyNameSplit)
    {
        ...
        if (_object.GetType().IsClass == false) return new StructAccessor(propertyInfo, PublicOnly);
        ...
        var setterDelegateType =
            typeof(Action<,>).MakeGenericType(propertyInfo.DeclaringType, propertyInfo.PropertyType);
    ...
    private static IAccessor CreateIAccessorWithType(object _object, string propertyNameSplit)
    {
        ...
        if ((_object as Type).IsClass == false) return new StructAccessor(propertyInfo, PublicOnly);
        ...
        var accessorType =
            typeof(StaticPropertyAccessor<,>).MakeGenericType(propertyInfo.DeclaringType, propertyInfo.PropertyType);
    ...
    private static IAccessor CreateIAccessorWithIndex(object _object, string propertyNameSplit)
    {
        ...
        var getterDelegateType =
            typeof(Func<,,>).MakeGenericType(propertyInfo.DeclaringType, typeof(int), propertyInfo.PropertyType);
        var getter = getInfo != null ? Delegate.CreateDelegate(getterDelegateType, getInfo) : null;

        var setterDelegateType =
            typeof(Action<,,>).MakeGenericType(propertyInfo.DeclaringType, typeof(int), propertyInfo.PropertyType);
        var setter = setInfo != null ? Delegate.CreateDelegate(setterDelegateType, setInfo) : null;

        var accessorType =
            typeof(IndexerAccessor<,>).MakeGenericType(propertyInfo.DeclaringType, propertyInfo.PropertyType);
    ...
    public static void InvokeGenericMethod(object classInstance, Type type, string methodName, params object[] args)
    {
        ...
        foreach (var arg in args) hash ^= arg.GetType().GetHashCode();
