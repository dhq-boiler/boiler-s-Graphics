...
public class MergeableOperation : IMergeableOperation, IOperationWithEvent
{
    ...
    /// </summary>
    public static IOperation MakeMerged(MergeableOperation prevOperation, MergeableOperation postOperation,
        bool checkKey = true)
    {
        if (checkKey)
        {
            if (postOperation.MergeJudge.CanMerge(prevOperation.MergeJudge))
                return new MergeableOperation(postOperation._rollForward, prevOperation._rollBack,
...
public class MergeableOperation<T> : IMergeableOperation, IOperationWithEvent
{
    ...
    ///     失敗した場合はOperation.Emptyが返却される
    /// </summary>
    public static IOperation MakeMerged(MergeableOperation<T> prevOperation, MergeableOperation<T> postOperation,
        bool checkKey = true)
    {
        if (checkKey)
        {
