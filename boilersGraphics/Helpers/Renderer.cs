...
namespace boilersGraphics.Helpers
{
    public static class Renderer
    {
        ...
        public static async Task<RenderTargetBitmap> RenderAsync(Rect? sliceRect, DesignerCanvas designerCanvas, DiagramViewModel diagramViewModel, BackgroundViewModel backgroundItem, DesignerItemViewModelBase mosaic = null)
        {
            ...
            await App.Current.Dispatcher.Invoke(async () =>
            {
                using (DrawingContext context = visual.RenderOpen())
                {
                    ...
                    //前景を描画
                    renderedCount += await RenderForegroundAsync(sliceRect, diagramViewModel, designerCanvas, context, backgroundItem, mosaic);
                }
                rtb.Render(visual);
                rtb.Freeze();
            });
        ...
        private static async Task<int> RenderForegroundAsync(Rect? sliceRect, DiagramViewModel diagramViewModel, DesignerCanvas designerCanvas, DrawingContext context, BackgroundViewModel background, DesignerItemViewModelBase mosaic = null)
        {
            var renderedCount = 0;
            var except = new SelectableDesignerItemViewModelBase[] { background, mosaic }.Where(x => x is not null);
            foreach (var item in diagramViewModel.AllItems.Value.Except(except).Where(x => x.IsVisible.Value))
        ...
        private static async Task<bool> RenderBackgroundViewModelAsync(Rect? sliceRect, DesignerCanvas designerCanvas, DrawingContext context, BackgroundViewModel background)
        {
            FrameworkElement view = default(FrameworkElement);
